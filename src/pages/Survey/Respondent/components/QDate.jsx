...
const QDate = (props) => {
    ...
    return (<Form.Item name={['question', "'" + question.id + "'"]} required={question.required === 1}>
      <h3>{question.serial ? question.serial + "-" : ""}{question.content}</h3>
       <DatePicker width="SM" label={question.content} name={['question', question.id]} placeholder="请选择日期" defaultValue={dayjs('1965-01-01', 'YYYY-MM-DD')} defaultPickerValue={dayjs('1965-01-01', 'YYYY-MM-DD')} onChange={onChange} format={"YYYY-MM-DD"} rules={[{ required: question.required === 1, message: '必填项' }]}/>
...
//# sourceMappingURL=QDate.jsx.map