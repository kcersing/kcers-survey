...
const MultipleChoice = (props) => {
    ...
    return (<Form.Item name={['question', "'" + question.id + "'"]} required={question.required === 1}>
      ...
      <ProFormCheckbox.Group onChange={onChange} layout="vertical" style={style} options={question.options.map(option => ({
            disabled: disabled,
            value: option.content,
            label: option.inputs !== 2 ? option.content :
                <>
              {option.content}...
              {value !== 0 && value && value.length > 0 && value.includes(option.content) && (<Input onChange={onChange1} variant="filled" placeholder="请输入..." style={{ width: 120, marginInlineStart: 12 }}/>)}
            </>,
        }))}/>
...
//# sourceMappingURL=MultipleChoice.jsx.map