...
const Respondent = () => {
    ...
    const rq = (question, parentname) => {
        ...
        {question && question.children && question.children.length > 0 && (<>
            {question.children.map((child, index) => (<>{rq(child, '')}</>))}
          </>)}
      </>);
    };
    const RenderQuestionControl = (question) => {
        if (question.type === 'h2') {
            return (<h2 style={{ width: 300 }}>
          <b>{question.content}</b>
        </h2>);
        }
        else if (question.type === 'h3') {
            return (<h3 style={{ width: 300 }}>
          <b>
            ...
            return (<QuestuinSun surveyId={surveyId} question={question} generateRandom={generateRandom} addRespondent={addRespondent} setCurrentNum={setCurrentNum} setCurrent={setCurrent}></QuestuinSun>);
        }
    };
    const respondent = () => {
        return (<QRespondent surveyId={surveyId} questions={questions} generateRandom={generateRandom} addRespondent={addRespondent} setCurrentNum={setCurrentNum} setCurrent={setCurrent}/>);
    };
    const renderThankYou = () => {
        return (<StepsForm.StepForm name={`key_${questions.length + 2}`} key={`key_${questions.length + 2}`}>
    ...
    return (<div className="respondent-container">
      ...
      <ProCard style={{ width: '100%', marginBlockStart: 16 }} boxShadow>
        <StepsForm
    // loading={loading}
    formRef={formRef} formMapRef={formMapRef} stepsProps={{
            direction: 'vertical',
            size: 'small',
            current: 1,
        }} formProps={{
        ...
        }} current={current} onFinish={(values) => {
            console.log(values);
          ...
          <>
            <Address surveyId={surveyId} questions={questions} generateRandom={generateRandom} addRespondent={addRespondent} setCurrentNum={setCurrentNum} setCurrent={setCurrent}/>
          </>

          {questions.map((question) => (<>
              {question.children.map((que) => (<StepsForm.StepForm style={{ width: 360 }} name={que.id} title={que.id} key={que.id} onFinish={(values) => {
                    console.log(values);
          ...
          <SFUpload surveyId={surveyId} questions={questions} generateRandom={generateRandom} addRespondent={addRespondent} setCurrentNum={setCurrentNum} setCurrent={setCurrent}/>
          {renderThankYou()}
        </StepsForm>
...
//# sourceMappingURL=index-st1.jsx.map
