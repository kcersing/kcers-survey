...
// 问题渲染组件 - 使用 memo 优化
// 导航按钮组件 - 使用 memo 优化
const NavigationButtons = memo(({ isFirstQuestion, isSubmitting, uploading, moveToPreviousQuestion, moveToNextQuestion, form }) => {
    return (<div className="survey-buttons">
      <Button type="default" icon={<ArrowLeftOutlined />} disabled={isFirstQuestion || isSubmitting} onClick={moveToPreviousQuestion}>
        上一题
      </Button>
      <Button type="primary" onClick={moveToNextQuestion} // 直接调用导航函数，不依赖表单提交
     loading={isSubmitting || uploading} style={{ marginLeft: 16 }}>
...
const Survey = () => {
    ...
    // 获取当前问题 - 只考虑根问题
    const getCurrentQuestion = useCallback((path) => {
    ...
    // 处理跳题 - 简化版本，只考虑根问题
    const handleJumpRules = useCallback((question, values) => {
    ...
    // 处理问题提交
    const handleQuestionSubmit = useCallback((values) => {
        console.log(5);
        const currentQuestion = getCurrentQuestion(currentPath);
        if (!currentQuestion)
            return;
    ...
    const handleSurveySubmit = useCallback(() => {
        ...
        // 验证必填问题 - 只考虑根问题
        const requiredQuestions = questions.filter(q => q.type !== 'h2' && q.type !== 'h3' && q.required);
    ...
    const formatProgress = useCallback(() => {
        ...
        const answeredCount = allQuestions.filter(q => answers[q.id] !== undefined &&
            answers[q.id][`question${q.id}`] !== undefined &&
            answers[q.id][`question${q.id}`] !== null).length;
    ...
    const handleUploadSuccess = useCallback((type, file) => {
        dispatch({ type: 'UPLOADING', payload: false });
        message.success('上传成功');
    }, []);
    const handleUploadError = useCallback((type, error) => {
        dispatch({ type: 'UPLOADING', payload: false });
    ...
    return (<div className="survey-container">
      <ProCard className="survey-card" title={currentQuestion?.content || survey.title || '问卷调查'} extra={<Text type="secondary">答题进度: {formatProgress()}</Text>} bordered={false}>
        <ProForm form={form} onFinish={handleQuestionSubmit} layout="vertical">
          ...
          <NavigationButtons isFirstQuestion={currentPath.length === 0} isSubmitting={surveyState.isSubmitting} uploading={surveyState.uploading} moveToPreviousQuestion={moveToPreviousQuestion} moveToNextQuestion={moveToNextQuestion} form={form}/>
        </ProForm>
      </ProCard>
    </div>);
};
