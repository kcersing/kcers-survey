...
import { ProForm, ProFormText, ProFormSelect, ProFormRadio, DragSortTable, } from '@ant-design/pro-components';
import { PlusOutlined, DeleteOutlined, EditOutlined, DragOutlined } from '@ant-design/icons';
import { listQuestion, createQuestion, updateQuestion, deleteQuestion, getSurvey } from '@/services/ant-design-pro/survey';
import { MenuOutlined } from '@ant-design/icons';
import { Button, Modal, message, Card, Space, Divider } from 'antd';
// 拖拽排序项
const SortableItem = SortableElement(({ question, onEdit, onDelete }) => (<tr>
...
const SurveyDesigner = ({ match }) => {
    ...
    const handleDragSortEnd1 = (beforeIndex, afterIndex, newDataSource) => {
    ...
    return (<div>
        <Card title={`问卷设计: ${survey.title || '加载中...'}`} bordered={false} extra={[
            ...
            <DragSortTable actionRef={actionRef} headerTitle="拖拽排序(默认把手)" columns={columns} rowKey="id" search={false} pagination={false} loading={loading} dataSource={questions} dragSortKey="sort" dragSortHandlerRender={dragHandleRender} onDragSortEnd={handleDragSortEnd1}/>
        ...
        {/* 问题编辑模态框 */}
        <Modal title={editingQuestion ? '编辑问题' : '添加问题'} open={visible} onCancel={() => setVisible(false)} footer={null} width={800}>
          <ProForm form={form} layout="vertical" initialValues={editingQuestion || { required: true, sort_order: questions.length + 1 }}>
            <ProFormText name="content" label="问题内容" rules={[{ required: true, message: '请输入问题内容' }]} rows={3}/>

            <ProFormSelect name="question_type" label="问题类型" onChange={(value) => setQuestionType(value)} options={[
            ...
            <ProFormRadio.Group name="required" label="是否必填" options={[
            { label: '是', value: true },
            { label: '否', value: false },
        ]}/>

            <ProFormText name="sort_order" label="排序" type="number" placeholder="请输入排序号"/>

            {/* 选项设置（针对单选题和多选题） */}
            {['single_choice', 'multiple_choice'].includes(questionType) && (<div>
                  <h3 className="font-medium mb-2">选项设置</h3>
                  {options.map((option, index) => (<div key={index} className="flex items-center mb-2">
                        <ProFormText value={option} onChange={(value) => {
                    const newOptions = [...options];
                    newOptions[index] = value;
                    setOptions(newOptions);
                }} placeholder={`选项 ${index + 1}`} style={{ marginRight: 8, flex: 1 }}/>
            ...
            {/* 矩阵设置（针对矩阵单选题） */}
            {questionType === 'matrix_single' && (<div>
                  <h3 className="font-medium mb-3">矩阵设置</h3>

                  <div className="mb-6">
                    <h4 className="font-medium mb-2">行设置</h4>
                    {matrixRows.map((row, index) => (<div key={index} className="flex items-center mb-2">
                          <ProFormText value={row} onChange={(value) => {
                    const newRows = [...matrixRows];
                    newRows[index] = value;
                    setMatrixRows(newRows);
                }} placeholder={`行 ${index + 1}`} style={{ marginRight: 8, flex: 1 }}/>
                          <Button type="danger" icon={<DeleteOutlined />} onClick={() => {
                    const newRows = [...matrixRows];
                    ...
                    <Button type="dashed" onClick={() => setMatrixRows([...matrixRows, ''])} style={{ width: '100%' }}>
                      <PlusOutlined /> 添加行
                    </Button>
                  </div>
                  ...
                  <div>
                    ...
                    {matrixColumns.map((column, index) => (<div key={index} className="flex items-center mb-2">
                          <ProFormText value={column} onChange={(value) => {
                    const newColumns = [...matrixColumns];
                    newColumns[index] = value;
                    setMatrixColumns(newColumns);
                }} placeholder={`列 ${index + 1}`} style={{ marginRight: 8, flex: 1 }}/>
                          <Button type="danger" icon={<DeleteOutlined />} onClick={() => {
                    const newColumns = [...matrixColumns];
                    ...
                    <Button type="dashed" onClick={() => setMatrixColumns([...matrixColumns, ''])} style={{ width: '100%' }}>
                      <PlusOutlined /> 添加列
                    </Button>
                  </div>
                </div>)}
...
//# sourceMappingURL=Designer.jsx.map