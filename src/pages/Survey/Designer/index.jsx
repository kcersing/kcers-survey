...
import { ProForm, ProFormRadio, ProFormSelect, ProFormText, ProCard, ProFormDigit, ProTable, ProFormGroup, ProFormList, ProFormTreeSelect, } from '@ant-design/pro-components';
import { Button, message, Modal, Divider } from 'antd';
import { listQuestion, getSurvey, createQuestion, updateQuestion, deleteQuestion, treeQuestion, } from '@/services/survey';
...
const Designer = () => {
    ...
    return (<>
      <ProCard title={`问卷设计: ${survey.title || '加载中...'}`} extra={[
        ...
        <ProTable actionRef={actionRef} columns={columns} rowKey="id" search={false} pagination={false} loading={loading} dataSource={questions} columnWidth={12} defaultExpandAllRows={true}/>
      </ProCard>

      {/* 问题编辑模态框 */}
      <Modal title={editingQuestion ? '编辑问题' : '添加问题'} open={visible} onCancel={() => setVisible(false)} footer={null} width={800}>
        <ProForm form={form} layout="vertical" initialValues={editingQuestion || { required: 1, questionType: 'single_choice', sort: questions ? questions.length + 1 : 0 }} submitter={{
            // 配置按钮文本
            searchConfig: {
                resetText: '重置',
                submitText: '提交',
          ...
          <ProFormTreeSelect name="parentId" label="上级问题" params={{}} request={async ({ keyWords }) => {
            const questionAll = await treeQuestion({ surveyId: surveyId, keywords: keyWords });
            return questionAll.data;
        }} fieldNames={[{ label: 'title', value: 'value', children: 'children' }]} style={{ width: '100%' }} placeholder="Please select" onChange={handleChange} showSearch={false}/>
          <ProFormText name="serial" label="编号"/>
          <ProFormText name="content" label="问题内容" rules={[{ required: true, message: '请输入问题内容' }]} rows={3}/>

          <ProFormSelect name="questionType" label="问题类型" onChange={(value) => setQuestionType(value)} options={[
            { label: '标题1', value: 'h2' },
            { label: '标题2', value: 'h3' },
          ...
          <ProFormDigit name="sort" label="排序" min={1} max={10} fieldProps={{ precision: 0 }} placeholder="请输入排序号"/>


          {questionType === 'single_choice' && (<>
              <ProFormList label="选项设置" copyIconProps={{ Icon: SnippetsOutlined }} initialValue={options} deleteIconProps={{ Icon: CloseOutlined }} name="options">
                  <ProFormGroup key="group">
                  <ProFormDigit name="serial" label="序号"/>
                  <ProFormText name="content" label="选项"/>

                  <ProFormRadio.Group name="inputs" label="是否可填写" value={1} options={[
                { label: '否', value: 1 },
                { label: '是', value: 2 },
            ]}/>
                </ProFormGroup>
          ...
          {questionType === 'multiple_choice' && (<>
            ...
            <ProFormDigit name="valueNumber" label="最多选中" fieldProps={{ precision: 0 }} placeholder="3"/>
            </>)}
