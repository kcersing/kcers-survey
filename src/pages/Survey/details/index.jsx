import { Card, Descriptions, Divider, Steps, Image } from 'antd';
    ...
    return (<PageContainer>
      <Card bordered={false}>
        <Descriptions title="信息" style={{
          ...
          <Descriptions.Item label="合照照片">{response ? response.pic?.map(v => {
        ...
        <div className={styles.title}>详情</div>
          ...
          <Card style={{
            marginBottom: 24,
              ...
              <Steps progressDot current={responseAnswers.length + 1} direction="vertical" items={responseAnswers.map(item => ({
            title: item.content,
            // 如果有子节点，递归转换
            description: <Card>
                        <ul>
