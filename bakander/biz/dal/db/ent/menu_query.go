...
func (_q *MenuQuery) QueryParams() *MenuParamQuery {
	query := (&MenuParamClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *MenuQuery) WithParams(opts ...func(*MenuParamQuery)) *MenuQuery {
	query := (&MenuParamClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MenuQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Menu, error) {
	...
	_spec.Node.Schema = _q.schemaConfig.Menu
...
func (_q *MenuQuery) loadRoles(ctx context.Context, query *RoleQuery, nodes []*Menu, init func(*Menu), assign func(*Menu, *Role)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		joinT.Schema(_q.schemaConfig.RoleMenus)
...
func (_q *MenuQuery) loadParams(ctx context.Context, query *MenuParamQuery, nodes []*Menu, init func(*Menu), assign func(*Menu, *MenuParam)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Menu)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (_q *MenuQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(_q.schemaConfig.Menu)
	ctx = internal.NewSchemaConfigContext(ctx, _q.schemaConfig)
