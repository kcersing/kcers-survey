...
type Client struct {
	...
	SurveyResponseAnswers *SurveyResponseAnswersClient
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	...
	case *SurveyResponseAnswersMutation:
		return c.SurveyResponseAnswers.mutate(ctx, m)
	case *TokenMutation:
...
func (c *SurveyClient) QueryQuestion(s *Survey) *SurveyQuestionQuery {
...
// SurveyResponseAnswersClient is a client for the SurveyResponseAnswers schema.
type SurveyResponseAnswersClient struct {
	config
}

// NewSurveyResponseAnswersClient returns a client for the SurveyResponseAnswers from the given config.
func NewSurveyResponseAnswersClient(c config) *SurveyResponseAnswersClient {
	return &SurveyResponseAnswersClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `surveyresponseanswers.Hooks(f(g(h())))`.
func (c *SurveyResponseAnswersClient) Use(hooks ...Hook) {
	c.hooks.SurveyResponseAnswers = append(c.hooks.SurveyResponseAnswers, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `surveyresponseanswers.Intercept(f(g(h())))`.
func (c *SurveyResponseAnswersClient) Intercept(interceptors ...Interceptor) {
	c.inters.SurveyResponseAnswers = append(c.inters.SurveyResponseAnswers, interceptors...)
}

// Create returns a builder for creating a SurveyResponseAnswers entity.
func (c *SurveyResponseAnswersClient) Create() *SurveyResponseAnswersCreate {
	mutation := newSurveyResponseAnswersMutation(c.config, OpCreate)
	return &SurveyResponseAnswersCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of SurveyResponseAnswers entities.
func (c *SurveyResponseAnswersClient) CreateBulk(builders ...*SurveyResponseAnswersCreate) *SurveyResponseAnswersCreateBulk {
	return &SurveyResponseAnswersCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *SurveyResponseAnswersClient) MapCreateBulk(slice any, setFunc func(*SurveyResponseAnswersCreate, int)) *SurveyResponseAnswersCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &SurveyResponseAnswersCreateBulk{err: fmt.Errorf("calling to SurveyResponseAnswersClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*SurveyResponseAnswersCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for SurveyResponseAnswers.
func (c *SurveyResponseAnswersClient) Update() *SurveyResponseAnswersUpdate {
	mutation := newSurveyResponseAnswersMutation(c.config, OpUpdate)
	return &SurveyResponseAnswersUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}
...
// UpdateOneID returns an update builder for the given id.
func (c *SurveyResponseAnswersClient) UpdateOneID(id int64) *SurveyResponseAnswersUpdateOne {
	mutation := newSurveyResponseAnswersMutation(c.config, OpUpdateOne, withSurveyResponseAnswersID(id))
	return &SurveyResponseAnswersUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for SurveyResponseAnswers.
func (c *SurveyResponseAnswersClient) Delete() *SurveyResponseAnswersDelete {
	mutation := newSurveyResponseAnswersMutation(c.config, OpDelete)
	return &SurveyResponseAnswersDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}
...
// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *SurveyResponseAnswersClient) DeleteOneID(id int64) *SurveyResponseAnswersDeleteOne {
	builder := c.Delete().Where(surveyresponseanswers.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &SurveyResponseAnswersDeleteOne{builder}
}

// Query returns a query builder for SurveyResponseAnswers.
func (c *SurveyResponseAnswersClient) Query() *SurveyResponseAnswersQuery {
	return &SurveyResponseAnswersQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeSurveyResponseAnswers},
		inters: c.Interceptors(),
	}
}

// Get returns a SurveyResponseAnswers entity by its id.
func (c *SurveyResponseAnswersClient) Get(ctx context.Context, id int64) (*SurveyResponseAnswers, error) {
	return c.Query().Where(surveyresponseanswers.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *SurveyResponseAnswersClient) GetX(ctx context.Context, id int64) *SurveyResponseAnswers {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *SurveyResponseAnswersClient) Hooks() []Hook {
	return c.hooks.SurveyResponseAnswers
}

// Interceptors returns the client interceptors.
func (c *SurveyResponseAnswersClient) Interceptors() []Interceptor {
	return c.inters.SurveyResponseAnswers
}

func (c *SurveyResponseAnswersClient) mutate(ctx context.Context, m *SurveyResponseAnswersMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&SurveyResponseAnswersCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&SurveyResponseAnswersUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&SurveyResponseAnswersUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&SurveyResponseAnswersDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown SurveyResponseAnswers mutation op: %q", m.Op())
	}
}
